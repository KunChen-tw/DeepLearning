...
N_STATES = 11 # 狀態個數，每個狀態都有 ACTIONS 的機率
ACTIONS = ['left', 'right'] #動作
EPSILON = 0.9   # The probability of choosing the optimal strategy, 剩0.1是用猜的 (試著跳出 q talbe 思維)
...
def update_env(S, episode, step_counter):
    ...
    if S == 'terminal':
        ...
        interaction = 'Episode %s: total_steps = %s (Fell into Cliff)' % (episode + 1, step_counter)
        print('\r{}'.format(interaction), end='')  # 顯示掉進懸崖的結果
        time.sleep(2)
        print('\n                                ', end='')  # 清空行
    else:
        env_list[S] = 'o'
